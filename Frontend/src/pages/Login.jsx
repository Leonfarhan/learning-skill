...
import Background from '/Login-Page.png';
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(error.response);  // Log respons kesalahan
      if (error.response && (error.response.status === 401 || error.response.status === 402)) {
        // Tampilkan pesan kesalahan kepada pengguna
        alert("Email/password yang Anda masukkan salah atau akun Anda belum terdaftar.");
      } else {
  ...
  return (
    <div className="flex justify-center items-center fixed">
      <div className="flex items-center justify-center h-screen w-screen" style={{ backgroundImage: `url(${Background})`, backgroundSize: 'cover' }}>

        <div className="flex flex-col w-full max-w-md px-12 py-8 bg-gray-800 rounded-lg shadow">
          <img
            src="src/assets/logo-text.svg"
            className="w-52 h-auto mx-auto my-8"
            alt="Logo"
          />
